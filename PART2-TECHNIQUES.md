...
**What's in it for you:** moves you can copy in under a minute each — every one deployed daily, rated by difficulty, indexed so you (or your agent) can jump straight to the problem you have right now.

Field-tested techniques organized by category. Each rated:
...
This is the same architecture as Simon Willison's [dual-LLM pattern](https://simonwillison.net/2023/Apr/25/dual-llm-pattern/) and DeepMind's [CaMeL](https://simonwillison.net/2025/Apr/11/camel/) (a privileged planner, a quarantined reader), and it satisfies Willison's [lethal trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/) rule by construction: the component exposed to untrusted content never simultaneously holds private-data access *and* an outbound channel. Prompt injection is not a solved problem — this pattern doesn't stop the injection, it takes away the injection's hands. Be precise about what remains: its voice.

**What the split does not remove:** the trust boundary doesn't disappear — it relocates onto the approver. The interpreter still authors the draft, the attacker's message is part of what shaped it, and the attacker is already on the thread the reply returns to. The approval step is the check doing the work, which makes two disciplines load-bearing. First, the approval surface must render the **source message next to the draft** — approving text you haven't compared against what provoked it is a reflex, not a review. Second, scope the interpreter's reads to the thread it is answering, so an injected draft cannot quote private content from anywhere else. The failure mode to respect is approval fatigue: the fortieth rubber-stamp is not a security review. (This paragraph exists because [Sarvex Jatasra](https://www.linkedin.com/in/sarvex/) made the boundary-relocation point in a public review of this pattern. He was right; the earlier text overclaimed.)

Scope note: this is a *personal-tooling* threat model — one approver, low volume, your own inbox. It does not transfer to enterprise agents: human approval doesn't survive enterprise volume, and "the approver is the boundary" is not an acceptable answer for systems handling other people's data. That problem needs policy enforcement on data flows (the CaMeL direction), not a person — it is being worked in the open at [AgenTrust-io.com](https://agentrust-io.com), an open source community this repo's author is heavily involved with.

Field-tested: this repo author's unattended inbox agent runs exactly this split — a read-only interpreter drafts replies at queue time; the control channel renders the full source message alongside the proposed draft for review; approval triggers a hard-coded send of exactly the surfaced draft, in-thread; a structural test asserts the interpreter file has no executor references.
...
![Who does what in a consolidation pass: the scheduled pass reads the corpus and writes a dated report, a person ticks the proposals worth keeping, and a separate command applies exactly those.](assets/diagrams/memory-consolidation.png)
...
![The status field as a state machine: idea into active, active cycling through blocked, and completion running out through done into archived.](assets/diagrams/project-pulse-states.png)
...
![Where the hook fires: an edit in a different project directory raises the context-switch warning and holds work until the departing project's tracker is updated.](assets/diagrams/focus-lock.png)
