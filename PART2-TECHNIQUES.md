...
Scope note: this is a *personal-tooling* threat model — one approver, low volume, your own inbox. It does not transfer to enterprise agents: human approval doesn't survive enterprise volume, and "the approver is the boundary" is not an acceptable answer for systems handling other people's data. That problem needs policy enforcement on data flows (the CaMeL direction), not a person.
